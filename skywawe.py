...
def get_system_info():
    ...
    IP_address = interface[ip_address.end():][6:19]
...
def parse_xml(Serial_ID,Mac_ID,Version_ID, address, device=0):
    #Parse XML files
    Network_file = ET.parse("/opt/Engine/Settings/Network.xml")
    Environment_file = ET.parse("/opt/Engine/Settings/Environment.xml")
...
def main():
    ...
    ap.add_argument('-s', '--serial', type=str, help='use Serial ID to re-configure xml files')
    ap.add_argument('-i', '--address', help='set given IP address for second or third device')
    ap.add_argument('-p', '--port', help='set starting port number for second or third device it reserve ten port for device')
    ap.add_argument('-o', '--order', help='set configuration for second or third device, 2 for second, 3 for third device')
    args = ap.parse_args()
    ...
    if args.serial:
       Network_file, Environment_file =  parse_xml(args.serial, Mac_ID, version, IP_address)
       Network_file.write("/opt/Engine/Settings/Network.xml")
       Environment_file.write("/opt/Engine/Settings/Environment.xml")
    if args.order:
        if args.order == '2':
            Network_file, Environment_file =  parse_xml(args.serial, Mac_ID, version, IP_address='192.168.1.242')
        elif args.order == '3':
            Network_file, Environment_file =  parse_xml(args.serial, Mac_ID, version, IP_address='192.168.1.243')
        elif args.order == '4':
            Network_file, Environment_file =  parse_xml(args.serial, Mac_ID, version, IP_address='192.168.1.244') 
        Network_file.write("/opt/Engine/Settings/Network.xml")
        Environment_file.write("/opt/Engine/Settings/Environment.xml")
    if args.version:
        pass
